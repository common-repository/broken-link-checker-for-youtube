...
== Description ==
Broken Link Checker for YouTube is a WordPress plugin that will automatically validate YouTube embeds found in your published posts content using the official API.

It does this by extracting video IDs from each post and querying the YouTube API to ensure those IDs are still valid. This is a MUST for any video site that wants to retain their visitors and not annoy them with broken content.
